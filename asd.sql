...
SELECT `p`.`id_pedido`, tp.descripcion AS 'tipoPedido', (p.fechaalta, '%d/%m/%Y %H:%i') AS fechaalta, `p`.`titulo`, `p`.`descripcion`, dep.descripcion AS 'dependenciaOrigen' FROM `pedido` AS `p` JOIN `tipopedido` AS `tp` ON `tp`.`id_tipopedido` = `p`.`id_tipopedido` JOIN `dependencia` AS `dep` ON `dep`.`id_dependencia` = `p`.`dependenciaorigen` WHERE `p`.`activo` = 1 LIMIT 50
SELECT p.id_pedido, tp.descripcion AS 'tipoPedido', p.fechaalta, p.titulo, p.descripcion, 
